...
class Attention(nn.Module):
    ...
    ################################################################################
    ################################ Added by szt ##################################
    def set_to_cond_v(self, meta_dim):
        from ldm.models.diffusion.networks_add import MLP
        hidden_dim = self.cross_attention_dim
        self.non_linear_op = 1
        self.reg_loss = 0
        self.to_cond_v = MLP(768, 1024, self.out_dim)
        #self.to_cond_k = MLP(self.out_dim, 1024, self.out_dim)
        #self.proj_pa = nn.Linear(768, self.out_dim)
    ################################ Added by szt ##################################
    ################################################################################
    ################################################################################
...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        if encoder_hidden_states is None:
            is_attention_map = 0
        #########################################################
        #print("encoder_hidden_states.shape:",encoder_hidden_states.shape) #1, 4, 64, 64
        if encoder_hidden_states is None:
            encoder_hidden_states = hidden_states
        elif attn.norm_cross:
            encoder_hidden_states = attn.norm_encoder_hidden_states(encoder_hidden_states)
        #print("encoder_hidden_states.shape:",encoder_hidden_states.shape) # [1, 4096, 320] or torch.Size([1, 5, 768])
        
        key = attn.to_k(encoder_hidden_states, *args)
        ###############    SZT   ################################
        
        value = attn.to_v(encoder_hidden_states, *args) # bs * word * hidden_dim # torch.Size([1, 5, 768])
        '''
        if attn.non_linear_op == 1 and is_attention_map == 1:
            
            pa = attn.to_cond_v(attn.proj_pa(pa))
            
            value_temp = attn.to_cond_v(value)
            attn.reg_loss = F.mse_loss(value, value_temp, reduction='mean')
            #print(attn.reg_loss)
            value = torch.concat([value_temp, pa], dim = 1)
            
            
            pa = attn.to_cond_k(pa)
            key_temp = attn.to_cond_k(key)
            attn.reg_loss += F.mse_loss(key, key_temp, reduction='mean')
            #print(attn.reg_loss)
            key = torch.concat([key_temp, pa], dim = 1)
        '''
        ############### version 1  ###############
        
        if attn.non_linear_op == 1:
            
            pa = attn.to_cond_v(pa).transpose(1, 2)
            #print(pa.shape, value.shape)
            value = F.scaled_dot_product_attention(
                pa, pa, value.transpose(1, 2), attn_mask=attention_mask, dropout_p=0.0, is_causal=False
            ).transpose(1, 2)
        ...
        if is_attention_map == 0:
            attention_probs = None
        else:
            #query.shape, key.shape, value.shape: torch.Size([1, 8, 1024, 80]) torch.Size([1, 8, 12, 80]) torch.Size([1, 8, 12, 80]), 8*80: hidden_dim, 1024: 32*32, width * height
            
            attention_probs = torch.matmul(query, key.transpose(-1, -2)).mean(dim = 1) # implementation 2
