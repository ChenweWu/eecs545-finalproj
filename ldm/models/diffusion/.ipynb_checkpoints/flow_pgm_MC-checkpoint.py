...
class BasePGM(nn.Module):
    ...
    def counterfactual(
        ...
        batch_size = list(obs.values())[0].shape[0]
        print("PAIRED OBS,INTV",obs,intervention)
...
class ChestPGM_MC(BasePGM):
    def __init__(self, args: Hparams):
        ...
        finding_net = DenseNN(1, [8, 16], param_dims=[6], nonlinearity=nn.LeakyReLU())
        ...
        self.finding_logits = nn.Parameter(np.log(1 / 6) * torch.ones(1, 6))
        if args.setup != "sup_pgm":
            ...
            self.encoder_a = ResNet18(num_outputs=2, context_dim=6, **kwargs)
            self.f = (
                lambda x: args.std_fixed * torch.ones_like(x)
                if args.std_fixed > 0
                else F.softplus(x)
    ...
    def model(self) -> Dict[str, Tensor]:
        ...
        # print("Defining finding distributino")
    ...
    def guide(self, **obs) -> None:
        with pyro.plate("observations", obs["x"].shape[0]):
            ...
            if obs["finding"] is None:
                ...
                qf_x = dist.Categorical(probs=f_prob).to_event(1)
    ...
    def model_anticausal(self, **obs) -> None:
        ...
        with pyro.plate("observations", obs["x"].shape[0]):
            ...
            qf_x= dist.Categorical(probs=f_probs)  # .to_event(1)
            # with pyro.poutine.scale(scale=0.5):shared
