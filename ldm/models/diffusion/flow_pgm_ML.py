...
from ldm.models.diffusion.layers import (  # fmt: skip
    CNN,
    MLP,
        ...
        for _ in range(num_particles):
            ...
            if hasattr(self, "discrete_variables"):  # hack for MIMIC
                ...
                if (
                    ...
                    # print("triggered")
                    # CO	PN	PX	PE
                    if "ED" not in intervention.keys():
                        counterfactuals['ED'] = obs['ED']
                    if "CO" not in intervention.keys():
                
                        counterfactuals["CO"] = obs["CO"]
                    if "PN" not in intervention.keys():
                        counterfactuals["PN"] = obs["PN"]
                    if "PX" not in intervention.keys():
                        counterfactuals["PX"] = obs["PX"]
                    if "PE" not in intervention.keys():
                        counterfactuals["PE"] = obs["PE"]
...
class ChestPGM_ML(BasePGM):
    def __init__(self, args: Hparams):
        ...
        self.discrete_variables = {            "ED": "binary",
            "CO": "binary",
            "PN": "binary",
            "PX": "binary",
            "PE": "binary",}
        # define base distributions
        for k in ["a", "f",'ED','CO','PN','PX','PE']:
            self.register_buffer(f"{k}_base_loc", torch.zeros(1))
            self.register_buffer(f"{k}_base_scale", torch.ones(1))
        # age spline flow
        ...
        ED_net = DenseNN(1, [8, 16], param_dims=[2], nonlinearity=nn.Sigmoid())
        CO_net = DenseNN(1, [8, 16], param_dims=[2], nonlinearity=nn.Sigmoid())
        PN_net = DenseNN(1, [8, 16], param_dims=[2], nonlinearity=nn.Sigmoid())
        PX_net = DenseNN(1, [8, 16], param_dims=[2], nonlinearity=nn.Sigmoid())
        PE_net = DenseNN(1, [8, 16], param_dims=[2], nonlinearity=nn.Sigmoid())
        # CO	PN	PX	PE
        self.ED_transform_GumbelMax = ConditionalGumbelMax(
            context_nn=ED_net, event_dim=0
        )
        self.CO_transform_GumbelMax = ConditionalGumbelMax(
            context_nn=CO_net, event_dim=0
        )
        self.PN_transform_GumbelMax = ConditionalGumbelMax(
            context_nn=PN_net, event_dim=0
        )
        
        self.PX_transform_GumbelMax = ConditionalGumbelMax(
            context_nn=PX_net, event_dim=0
        )
        
        self.PE_transform_GumbelMax = ConditionalGumbelMax(
            context_nn=PE_net, event_dim=0
        )
    ...
    def model(self) -> Dict[str, Tensor]:
        ...
        ED_dist_base = dist.Gumbel(self.ED_base_loc, self.ED_base_scale).to_event(1)
        ED_dist = ConditionalTransformedDistributionGumbelMax(
            ED_dist_base, [self.ED_transform_GumbelMax]
        ).condition(age)
        ED = pyro.sample("ED", ED_dist)
    # Code for CO
        CO_dist_base = dist.Gumbel(self.CO_base_loc, self.CO_base_scale).to_event(1)
        CO_dist = ConditionalTransformedDistributionGumbelMax(
            CO_dist_base, [self.CO_transform_GumbelMax]
        ).condition(age)
        CO = pyro.sample("CO", CO_dist)

        # Code for PN
        PN_dist_base = dist.Gumbel(self.PN_base_loc, self.PN_base_scale).to_event(1)
        PN_dist = ConditionalTransformedDistributionGumbelMax(
            PN_dist_base, [self.PN_transform_GumbelMax]
        ).condition(age)
        PN = pyro.sample("PN", PN_dist)

        # Code for PX
        PX_dist_base = dist.Gumbel(self.PX_base_loc, self.PX_base_scale).to_event(1)
        PX_dist = ConditionalTransformedDistributionGumbelMax(
            PX_dist_base, [self.PX_transform_GumbelMax]
        ).condition(age)
        PX = pyro.sample("PX", PX_dist)

        # Code for PE
        PE_dist_base = dist.Gumbel(self.PE_base_loc, self.PE_base_scale).to_event(1)
        PE_dist = ConditionalTransformedDistributionGumbelMax(
            PE_dist_base, [self.PE_transform_GumbelMax]
        ).condition(age)
        PE = pyro.sample("PE", PE_dist)

        # Returning all values including new ones
